...
## Creating unit tests
- When creating unit tests for a function, treat it as a black box.
- Analyze only the function's parameters and what the function returns.
...
- Test Structure (Arrange-Act-Assert)
  - All unit tests MUST follow the Arrange-Act-Assert (AAA) structure.
  - Exactly one action is allowed in the Act phase.
  - Each test should verify a single behavior.
  - A single function may (and often should) have multiple tests to cover distinct behaviors.
  - Assertions must validate observable behavior, not implementation details.
- Test Isolation
  - Unit tests MUST NOT use: real databases, network calls, filesystem access
  - External dependencies must be replaced with: mocks, stubs, fakes
  - Tests must run fully in memory and in isolation.
