...
- Subject under test: visual state communication for mode/status indicators, active sort summary, row markers, and edited-cell marker during staged write activity.
- Expected result: required visual indicators are shown deterministically as sort and staged changes are created in records view.
...
| S3 | Press `Shift+S`, choose column `name`, choose direction `ASC`, then confirm and inspect status line. | Status line includes active sort summary for selected table. | `A3` |
