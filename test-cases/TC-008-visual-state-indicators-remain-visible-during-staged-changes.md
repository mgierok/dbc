...
| S2 | Select table `categories`, press `Enter` to open records view, then inspect status line. | Status line shows clean-mode visual state and includes persisted-record count + pagination summaries for current table context. | `A2` |
...
| S4 | Press `e` to enter field-focus mode and edit one existing cell value; confirm edit popup. | Edited row is visibly marked and mode indicator switches to dirty write mode. | `A4` |
