...
| S2 | Select table `products`, press `Enter` to open records view, then press `Shift+F`. | Filter popup opens and starts at column-selection step. | `A2` |
...
| S5 | Open filter popup again (`Shift+F`), choose `category_id`, operator `=`, value `1`, then confirm. | New filter replaces prior filter and only one active filter remains for `products`. | `A5` |
