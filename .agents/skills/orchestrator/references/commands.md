...
Use padded IDs:
- `[prd-id]`: three digits (for example `001`)
- `[task-id]`: two digits (for example `01`)
...
```bash
rg --files .tasks | rg "^\\.tasks/PRD-[0-9]{3}-" | rg -v -- "-TASK-" | sort -V
```
...
```bash
last_id="$(rg --files .tasks | rg "^\\.tasks/PRD-[0-9]{3}-" | rg -v -- "-TASK-" | sed -E 's#\\.tasks/PRD-([0-9]{3})-.*#\\1#' | sort -n | tail -1)"
...
```bash
rg --files .tasks | rg "^\\.tasks/PRD-[prd-id]-TASK-[0-9]{2}-" | sort -V
```
...
```bash
rg -n "^- Status:" .tasks/PRD-[prd-id]-TASK-[0-9]{2}-*.md
```
...
```bash
rg -n "^- Status: READY$" .tasks/PRD-[prd-id]-TASK-[0-9]{2}-*.md
```
...
```bash
rg -n "^- Task ID:|^- Status:" .tasks/PRD-[prd-id]-TASK-[0-9]{2}-*.md | sort -V
```
