...
9. `:help` / `:h` routing behavior:
   ...
   - popup summary line and content sections (`Supported Commands`, `Supported Keywords`) are generated from registry functions (`runtimeHelpPopupSummaryLine`, `runtimeHelpPopupContentLines`) so copy stays aligned with active bindings and command aliases,
...
14. Selector browse/form/delete context lines are generated by input-registry helper functions (`selectorContextLinesBrowseDefault`, `selectorContextLinesBrowseFirstSetup`, `selectorFormSwitchLine`, `selectorFormSubmitLine`, `selectorDeleteConfirmationLine`) to keep on-screen hints synchronized with selector key handling.
15. Unsupported runtime commands keep existing fallback:
...
- Help popup text and runtime/selector shortcut hint lines are generated from registry-backed helper functions instead of duplicated inline literals.
