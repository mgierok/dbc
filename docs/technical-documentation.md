...
5. For selector-first path (no direct-launch argument), selector UI supports in-session config management (add/edit/delete with delete confirmation) and refreshes entries from config store after each successful mutation.
   ...
   Selector main content omits inline shortcut/legend rows and renders a right-aligned `Context help: ?` hint; selector shortcuts are available via `?` help overlay.
...
14. Runtime popup rendering standardization:
   ...
   - shared popup spec includes top-border title rendering, summary row, one-character horizontal side padding for content rows, optional selectable list rows, optional scroll window/indicator, width clamping, and minimum popup height set to `40%` of terminal height,
   - `View()` renders runtime popup overlays via `centerBoxLines` for help, filter, sort, edit, and confirm popup variants.
15. Runtime status-bar composition includes persisted-record page summary (`Records: current/total`, `Page: current/total`) and uses split rendering (`renderStatusWithRightHint`) so the right-aligned `Context help: ?` hint remains visible under narrow widths.
    Runtime status output is wrapped by `renderStatusBox` as an independent 3-row frame with one-character horizontal side padding in the content row.
16. Selector popup rendering behavior:
   - selector box uses top-border title rendering, one-character horizontal side padding, and minimum height set to `40%` of terminal height,
   - selector main content (browse/form/delete) omits inline shortcut and legend rows,
   - selector main content keeps right-aligned `Context help: ?` in the bottom content row,
   - selector `?` opens a dedicated context-help overlay that captures the current selector mode (`browse`, `add/edit`, `delete-confirm`) and renders only mode-relevant shortcut lines,
   - selector help overlay supports scroll navigation (`j/k`, `Ctrl+f`/`Ctrl+b`, `g`/`G`) and closes on `Esc`.
17. Runtime and selector UTF symbols are centralized in `internal/interfaces/tui/iconography.go`; renderers and tests consume shared constants for insert/edit/delete markers (`✚`, `✱`, `✖`) and existing indicators (`ℹ`, `⚙`, `⌨`, `↑`, `↓`).
18. Unsupported runtime commands keep existing fallback:
