...
- A successful runtime database switch preserves only app-session state such as the current `:set limit=<n>` page limit; table selection, current view, filter/sort state, and record selection reset for the newly opened database.
...
| Context | Controls |
| --- | --- |
| Runtime commands | `:config` / `:c`, `:help` / `:h`, `:w` / `:write`, `:wq`, `:quit` / `:q`, `:quit!` / `:q!`, `:set limit=<n>` |
